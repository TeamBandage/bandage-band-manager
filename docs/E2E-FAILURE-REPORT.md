...
| 조치 | D-01~D-03: BD-273 / D-04: BD-275 |
...
| **D-04** | 존재하지 않는 imageKey 이미지 등록 성공 | **Low** | 깨진 이미지 참조 데이터 생성 | S3 객체 존재 미검증 | J-08 |
...
## D-04. 존재하지 않는 imageKey로 이미지 등록 성공 (Low)
...
- 목록 조회 시 로딩 실패하는 이미지가 섞이며, 원인 추적이 어렵습니다.

포스터만의 문제가 아닙니다. presign 발급 key를 그대로 저장하는 **세 지점 모두** 동일했습니다.

| 위치 | 대상 |
|---|---|
| `PerformancePosterService.createPoster` | 공연 포스터 |
| `BandService.createBand` / `updateBand` | 밴드 프로필 이미지 |
| `MemberService.updateMemberInfo` | 회원 프로필 이미지 |

### 조치 (BD-275, A안 채택)

등록 시 S3 `headObject`로 객체 존재를 확인하고, 없으면 `400 IMAGE_NOT_UPLOADED`를 반환합니다.

| 항목 | 내용 |
|---|---|
| 추가 | `S3Client` 빈 (기존에는 `S3Presigner`만 등록되어 있었음) |
| 추가 | `S3ObjectValidator.requireExists(objectKey)` — 세 도메인 공통 사용 |
| 인프라 | IAM 정책에 `s3:GetObject`(HeadObject) 권한 **필수** — 없으면 이미지 등록이 전부 502 로 실패 |
| 트레이드오프 | 등록 API당 S3 왕복 1회 추가. 이미지 등록은 빈도가 낮아 수용 가능하다고 판단 |
| 실패 구분 | 입력 문제 `400`, 저장소 장애 `502` 로 분리 |

**실패 응답 구분** — 클라이언트 입력 문제와 시스템 외부 장애를 상태 코드로 나눕니다.

| 상황 | 응답 | 에러 코드 |
|---|---|---|
| 업로드되지 않은 key | `400` | `IMAGE_NOT_UPLOADED` |
| S3 장애·IAM 권한 누락 등 확인 불가 | `502` | `IMAGE_STORAGE_UNAVAILABLE` |

저장소가 죽은 상태에서 등록을 허용해도 그 이미지는 어차피 조회되지 않으므로, 폴백으로 통과시키지 않고 즉시 실패시켜 클라이언트가 재시도하게 합니다.

#### 검토 후 제외한 대안

| 대안 | 제외 사유 |
|---|---|
| B. 발급 key를 Redis에 보관 후 대조 | S3 호출이 없고 key 소유권까지 검증되지만, **실제 객체 존재를 보장하지 못해** 문제를 절반만 해결 |
| C. S3 이벤트 업로드 완료 콜백 | 가장 견고하나 이벤트·큐 인프라 구성과 스키마·클라이언트 계약 변경이 필요. Low 심각도 대비 과도 |
...
| 4 | **D-04** imageKey 미검증 | BD-275 에서 A안(headObject 검증)으로 처리 |
