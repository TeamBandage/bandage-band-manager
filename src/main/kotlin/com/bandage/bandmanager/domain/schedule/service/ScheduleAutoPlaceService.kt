...
class ScheduleAutoPlaceService(
    ...
    fun autoPlaceScheduleBlocks(
         ...
         * 가용 판정: 해당 그룹의 참여 멤버 전원이 확보 구간 전체에 가용해야 한다.
         *            가용성 미등록 멤버는 제약 없음(항상 가용)으로 본다.
         * maxJamsPerDay / maxEmptySlotsBetweenJams: 곡이 아니라 "모임" 단위로 센다.
         *            그룹이 만든 연속 블록들은 한 자리에 모인 것이므로 그날의 잼 횟수 1 로 센다.
         *            (블록 개수로 세면 곡이 많을수록 잼 횟수가 늘어 필드명과 의미가 어긋난다)
         *            maxEmptySlotsBetweenJams 도 그룹 내부의 빈틈이 아니라 서로 다른 모임 사이의 간격을 본다.
         ...
         *     이 구간은 그룹 전체가 쓸 자리이며, 배치 시점에 트랙 수만큼 잘려 블록이 된다.
         ...
         *     (그룹이 한 회차에 배치되면 그룹 내 모든 트랙이 각각 +1 된다)
