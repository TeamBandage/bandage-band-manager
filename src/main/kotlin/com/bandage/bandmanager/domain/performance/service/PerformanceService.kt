...
import com.bandage.bandmanager.domain.performance.dto.res.PerformanceBandMemberSummary
import com.bandage.bandmanager.domain.performance.dto.res.PerformanceBandSummary
...
import com.bandage.bandmanager.domain.performance.dto.res.PerformanceSetlistSummary
...
class PerformanceService(
    ...
    /**
     * 공연에 묶인 셋리스트 ID 를 반환한다(BD-264).
     * 공연 정보는 공개 정보이므로 참여 여부를 따지지 않는다(BD-279).
     */
    fun getSetlistIds(performanceId: UUID): List<UUID> = getPerformance(performanceId).setlists.map { it.setlistId }
